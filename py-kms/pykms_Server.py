...
def server_check():
        ...
        if diff := set(hexstr).symmetric_difference(set(hexsub)):
...
def server_create():
        ...
        loggersrv.info(
            f"HWID: {deco(binascii.b2a_hex(srv_config['hwid']), 'utf-8').upper()}"
        )
        return server
...
def server_with_gui():
        ...
        root.title(f'{pykms_GuiBase.gui_description} ({pykms_GuiBase.gui_version})')
        root.mainloop()
